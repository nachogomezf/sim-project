...
simulate_one_insurance_antithetic <- function(c0, n0, a, tl, lambda, mu, nu) {
  ...
  while (t < tl) {
    ...
    if (u < lambda * n / (lambda * n + mu * n + nu)) {
      ...
      if (c_anti > 0) { # If the capital is positive for antithetic, we can perform a claim
    ...
    # Stop if both capitals becomes negative, if only one does, we keep going with the other
...
```{r funsim}
set.seed(42)
res <- simulate_one_insurance_antithetic(c0 = 1000, n0 = 100, a = 100, tl = 100, 
...
```{r funsim_antithetic}
set.seed(42)
res <- simulate_insurance_antithetic(c0 = 1000, n0 = 100, a = 100, tl = 100, 
                          lambda = 0.1, mu = 0.1, nu = 0.1, MC = 10000)
res
```
